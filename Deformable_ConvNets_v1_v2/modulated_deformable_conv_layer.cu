...
void ModulatedDeformableConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      modulated_deformable_im2col_gpu<Dtype>(bottom_data + n*this->bottom_dim_, //data_col
                                          offset + n*this->input_offset_dim_,//offset
										  mask + n*this->input_mask_dim_,//mask
                                          bottom[0]->shape(1),
...
template <typename Dtype>
void ModulatedDeformableConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
     ...
     // Bias gradient, if necessary.
      ...
      if (this->param_propagate_down_[0] || propagate_down[0]) {
        ...
        bottom[2]->scale_diff(0);//data protect
        Dtype * bottom_mask_diff = bottom[2]->mutable_gpu_diff();

        for (int n = 0; n < this->num_; ++n) {
          if (this->param_propagate_down_[0]) {            
              modulated_deformable_im2col_gpu<Dtype>(
                   bottom_data + n*this->bottom_dim_, //data_col
                    bottom_offset + n*this->input_offset_dim_,//offset
					bottom_mask + n*this->input_mask_dim_,//mask
          ...
          if (propagate_down[0]) {
            ...
            // gradient w.r.t. input offset
              modulated_deformable_col2im_coord_gpu<Dtype>(this->col_buffer_.gpu_data(),
                bottom_data + n*this->bottom_dim_,
                bottom_offset + n*this->input_offset_dim_,
				bottom_mask + n*this->input_mask_dim_,
                ...
                this->kernel_shape_.cpu_data()[0],this->kernel_shape_.cpu_data()[1],
                  ...
                  this->deformable_group_, bottom_offset_diff + n*this->input_offset_dim_, bottom_mask_diff + n*this->input_mask_dim_);
              // gradient w.r.t. input data
              modulated_deformable_col2im_gpu<Dtype>(this->col_buffer_.gpu_data(),
                bottom_offset + n*this->input_offset_dim_,
				bottom_mask +n*this->input_mask_dim_,
                this->conv_in_channels_,this->conv_input_shape_.cpu_data()[1],this->conv_input_shape_.cpu_data()[2],this->col_buffer_.shape(0),
                this->kernel_shape_.cpu_data()[0],this->kernel_shape_.cpu_data()[1],
                this->pad_.cpu_data()[0],this->pad_.cpu_data()[1],this->stride_.cpu_data()[0],this->stride_.cpu_data()[1],
                        this->dilation_.cpu_data()[0],this->dilation_.cpu_data()[1],this->deformable_group_,bottom_diff+ n*this->input_offset_dim_+ n*this->input_mask_dim_);
...
INSTANTIATE_LAYER_GPU_FUNCS(ModulatedDeformableConvolutionLayer);
