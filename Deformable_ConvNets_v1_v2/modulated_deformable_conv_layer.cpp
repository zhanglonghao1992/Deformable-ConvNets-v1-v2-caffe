...
#include <iostream>
#include "caffe/layers/modulated_deformable_conv_layer.hpp"
using namespace std;
namespace caffe {
template <typename Dtype>
int ModulatedDeformableConvolutionLayer<Dtype>::input_shape(int i) {
   return (*(this->bottom_shape_))[this->channel_axis_ + i];
  }


template <typename Dtype>
void ModulatedDeformableConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  this->input_mask_dim_ = bottom[2]->count(this->channel_axis_);
...
template <typename Dtype>
void ModulatedDeformableConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void ModulatedDeformableConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {


  // Configure the kernel size, padding, stride, and inputs.
  ModulatedDeformableConvolutionParameter conv_param = this->layer_param_.modulated_deformable_convolution_param();
  ...
  this->num_output_ = this->layer_param_.modulated_deformable_convolution_param().num_output();
  CHECK_GT(this->num_output_, 0);
  this->group_ = this->layer_param_.modulated_deformable_convolution_param().group();
  this->deformable_group_ = this->layer_param_.modulated_deformable_convolution_param().deformable_group();
  ...
  << "Number channels of offset should be kernel_h*kernel_w*deformance*2";

  CHECK_EQ(bottom[1]->shape(2), bottom[0]->shape(2))
  << "Height and width of deformable conv layer and offset should be equal";

  CHECK_EQ(bottom[2]->shape(1), kernel_shape_data[0] * kernel_shape_data[1] * this->deformable_group_)
	  << "Number channels of offset should be kernel_h*kernel_w*deformance";

  CHECK_EQ(bottom[2]->shape(2), bottom[0]->shape(2))
	  << "Height and width of deformable conv layer and mask should be equal";
...
// cout<<bottom[0]->shape_string()<<endl;
  ...
  << "Height and width of deformable conv layer and offset should be equal";
  CHECK_EQ(bottom[2]->shape(3), bottom[0]->shape(3))
	  << "Height and width of deformable conv layer and mask should be equal";
  if (reverse_dimensions()) {
  ...
  this->bias_term_ = this->layer_param_.modulated_deformable_convolution_param().bias_term();
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.modulated_deformable_convolution_param().weight_filler()));
    weight_filler->Fill(this->blobs_[0].get());
    // If necessary, initialize and fill the biases.
    if (this->bias_term_) {
      this->blobs_[1].reset(new Blob<Dtype>(bias_shape));
      shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
          this->layer_param_.modulated_deformable_convolution_param().bias_filler()));
...
#ifdef CPU_ONLY
STUB_GPU(ModulatedDeformableConvolutionLayer);
#endif
INSTANTIATE_CLASS(ModulatedDeformableConvolutionLayer);
REGISTER_LAYER_CLASS(ModulatedDeformableConvolution);
